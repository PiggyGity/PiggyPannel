...
class Product extends DataLayer
{
    ...
    {
        $sql = "INSERT INTO Db_Tank41.dbo.Charge_Money (ChargeID, UserName, Money, Date, CanUse, PayWay, NeedMoney, IP, NickName) VALUES (?,?,?,GETDATE(),?,?,?,?,?)";
    ...
    public function SendRewardRecharge($uid, $pid)
    {
        ...
        foreach ($reward_list as $reward) {
            $sql = "INSERT INTO Db_Tank41.dbo.Sys_Users_Goods( UserID, BagType,TemplateID, Place, Count, IsJudge, Color, IsExist, StrengthenLevel, AttackCompose, DefendCompose, LuckCompose, AgilityCompose, IsBinds, BeginDate, ValidDate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?, getdate(),?)";
        ...
        foreach ($list as $itemID) {
            $sql = "INSERT INTO Db_Tank41.dbo.User_Messages( SenderID, Sender, ReceiverID, Receiver, Title, Content, SendTime, IsRead, IsDelR, IfDelS, IsDelete, Annex1, Annex2, Gold, Money, IsExist,Type,Remark) VALUES (?,?,?,?,?,?,getdate(),?,?,?,?,?,?,?,?,?,?,?)";
